...
namespace TarodevController {
    ...
    public class PlayerMovement : MonoBehaviour, IPlayerController {
        ...
        private void Update() {
            ...
            if (UnityEngine.Input.GetKeyDown(KeyCode.E) && spiritMode) {
...
private void CalculateJump() {
    ...
    if (Input.JumpDown && (CanUseCoyote || HasBufferedJump)) {
        ...
        hasDoubleJumped = false; // Reset the double jump flag when jumping from the ground
        JumpingThisFrame = true;
    }
    // Double jump if: not grounded && not already double jumped && not using coyote time
    else if(Input.JumpDown && !_colDown && !hasDoubleJumped && !_coyoteUsable) {
        _currentVerticalSpeed = _jumpHeight;
